...
class RefCOCOMapper:
    ...
    def __init__(
        ...
        *,
        ...
        cache_key = ("default_instances", default_inst_path)
        cached_default = self._INSTANCE_DATA_CACHE.get(cache_key)
        if cached_default is not None:
            self.id_to_ann = cached_default.get("annotations", {})
            self.preload_source = cached_default.get("path")
            source = self.preload_source or default_inst_path
            print(
                f"[RefCOCOMapper] Using cached {len(self.id_to_ann)} instance annotations from {source}"
            )
        elif os.path.exists(default_inst_path):
            try:
                ...
                self.preload_source = default_inst_path
                self._INSTANCE_DATA_CACHE[cache_key] = {
                    "annotations": self.id_to_ann,
                    "path": self.preload_source,
                }
                print(
    ...
    def _synthesize_mask(self, dataset_dict):
        ...
        for entry in status_log:
            ...
            if "bbox" in tokens:
                bbox_instances += 1
            if "synthetic" in tokens:
                synthetic_instances += 1

        total_instances = len(per_instance_masks)
        if synthetic_instances > 0 and decoded_instances == 0 and bbox_instances == 0:
            mode = "SYNTHETIC"
        elif decoded_instances > 0 and total_instances > 1:
            mode = "MERGED"
        elif decoded_instances > 0:
            mode = "MASK"
        elif bbox_instances > 0:
            mode = "BBOX"
        elif synthetic_instances > 0:
            mode = "SYNTHETIC"
        else:
            mode = "UNKNOWN"
        ...
        dataset_dict["mask_status"] = {
            ...
            "mode": mode,
            "ann_ids": ann_ids,
            "preload_source": self.preload_source,
        }
        image_id = dataset_dict.get("image_id", "unknown")
        logger.info(
            "[RefCOCOMapper] sample %s | mode: %s | mask.shape (%d, %d) | sum=%d",
            image_id,
            mode,
            height,
            width,
